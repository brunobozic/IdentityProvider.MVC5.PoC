...
namespace IdentityProvider.UI.Web.MVC5
{
    ...
    ///     see the Note under Remarks about a search like *.js
    ///     https://msdn.microsoft.com/en-us/library/wz42302f%28v=vs.110%29.aspx
    ///     3. BundlerHelper has the same problem (feature) as BundleConfig. If you change or add files to the settings.json
    ///     file
    ...
    ///     Finally it is very tempting to add comments to the settings.json file. While Json.Net handles comments Grunt
    ///     doesn't, and fails silently!
    /// </summary>
    public static class BowerBundlerHelper
    {
        ...
        ///     The name of the setting.json property containing the list of Css file to include.
        ///     defaults to main Css file
        /// </param>
        ...
        public static MvcHtmlString HtmlCssCached(this HtmlHelper helper, string bundleName, bool? forceState = null)
        {
            return forceState == null
                ? IncludeCache.GetOrAdd(bundleName,
                    setup => CreateHtmlIncludes(helper, bundleName, CssOrJs.Css, forceState))
        ...
        /// <param name="helper"></param>
        /// <param name="bundleName">
        ///     The name of the setting.json property containing the list of JavaScript file to include.
        ///     defaults to main js file
        /// </param>
        /// <param name="forceState">if not null then true forces into debug state and false forces production state</param>
        /// <returns></returns>
            ...
            bool? forceState = null)
        {
            return forceState == null
                ? IncludeCache.GetOrAdd(bundleName,
                    setup => CreateHtmlIncludes(helper, bundleName, CssOrJs.Js, forceState))
        ...
        ///     The format of how the cacheBuster value is added is controlled by the 'StaticFileCaching' in the
        ///     BunderForBower.json file.
        ...
        /// <param name="precalculatedCacheBuster">
        ///     if null then a cachebusting value is calculated based on the file content (cached locally for performance)
            ...
            string precalculatedCacheBuster = null)
        {
            if (precalculatedCacheBuster == null)
                precalculatedCacheBuster =
                    StaticCachebusterCache.GetOrAdd(relFilePath, setup => GetChecksumFromRelPath(relFilePath));
        ...
        public static string GetChecksumBasedOnFileContent(string absFilePath)
        {
            using (var stream = File.OpenRead(absFilePath))
            {
                ...
                return base64.Replace("/", "_").Replace("+", "-")
                    .Substring(0, base64.Length - 1); //make valid HTTP parameter string
            }
        }
